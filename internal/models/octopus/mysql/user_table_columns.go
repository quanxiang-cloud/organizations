...
func (u *userTableColumnsRepo) GetFilter(ctx context.Context, db *gorm.DB, status, attr int) ([]octopus.UserTableColumns, map[string]string) {
	...
	useColum := octopus.UseColumns{}
	if status == 0 {
		db = db.Where("id in (select column_id from " + useColum.TableName() + ")")
	} else {
		db = db.Where("id in (select column_id from "+useColum.TableName()+" where viewer_status=?)", status)
	}
